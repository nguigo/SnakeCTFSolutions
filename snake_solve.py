...
from miasm.expression.expression           import ExprInt, ExprId, ExprMem, ExprAssign
...
def stop_exec(jitter):
  ...
  for bbl, model in dse.new_solutions.items():
    ...
    if bbl_addr == 0x140004C07:
      ustr = current.decode('UTF-16')
      print(f'\tHit success bbl at {bbl_addr:08X} with tentative password: "{ustr:s}"')
      raise FinishException(current)
    else:
      print(f'\tHit new bbl at {bbl_addr:08X} with: {unicode_string(current):s}')
...
def main():
  ...
  # Begin run
  password = bytearray(b'\xFF\xFF') # Just one bogus unicode char to start with
  todo = [password]
  index = 0
  new_run = False
  while todo:
    ...
    for i, c in enumerate(current):
      sb.jitter.vm.set_u8(0x20000018+i, int(c))
    ...
    try:
      print(('-'*40 + ' NEWRUN\n' if new_run else '') + f'CONCRETE: {unicode_string(current):s}')
      sb.jitter.continue_run()
    except FinishException as fini:
      password = fini.password+bytearray(b'\xFF\xFF')
      todo = [password]
      index+=2
      new_run = True
      continue
